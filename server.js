...
app.get('*', async (c) => {
    try {
        ...
        // =========================================================
        // 🛠 KHU VỰC CĂN CHỈNH TỌA ĐỘ (Hãy sửa các số này nếu vẫn lệch)
        // =========================================================
        // Kích thước chuẩn của file ảnh frame-momo-qr.png của bạn
        const frameWidth = 600; 
        const frameHeight = 800;

        // 1. Tọa độ của vùng mã QR (Đã tinh chỉnh nhỏ lại dựa trên ảnh của bạn)
        const destX = 140;       // Dịch sang phải (Cũ là 125)
        const destY = 80;        // Dịch xuống dưới (Cũ là 65)
        const destWidth = 320;   // Thu nhỏ QR lại để lọt lòng viền xanh (Cũ là 350)
        const destHeight = 320;

        // 2. Tọa độ chữ
        let textStartY = 590;    // Đẩy cụm chữ xuống sâu hơn để né logo Napas (Cũ là 530)
        const lineSpacing = 30;  // Khoảng cách giữa các dòng chữ

        // 3. Chuỗi Base64 Ảnh (Dán mã của bạn vào đây)
        ...
        // =========================================================
        // KHỞI TẠO SVG - SỬA LỖI RESPONSIVE CHO IPHONE
        // Sử dụng width="100%" height="100%" và preserveAspectRatio
        // =========================================================
        ...
        const origW = 200, origH = 200; // Thay bằng size thật của logo2.png nếu cần
